...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addRoleOption(option =>
      option.setName("الرتبة").setDescription("الرتبة المراد إعطاؤها أو سحبها").setRequired(true)
    )
    ...
    .addUserOption(option =>
      option.setName("العضو").setDescription("العضو المحدد (مطلوب فقط إذا اخترت عضو محدد)").setRequired(false)
    )
    .addStringOption(option =>
  ...
  async execute(interaction) {
    try {
      ...
      if (target === "single") {
        ...
        // ✅ LOG
        if (isAdding) {
          discordLog.logRoleAdd(interaction.guild, { moderator: interaction.user, target: targetUser, role }).catch(() => {})
        } else {
          discordLog.logRoleRemove(interaction.guild, { moderator: interaction.user, target: targetUser, role }).catch(() => {})
        }
