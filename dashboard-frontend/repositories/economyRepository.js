...
async function createUser(userId) {
    try {
        ...
        const result = await databaseSystem.query(
            ...
            VALUES ($1, 0, 0, 0, $2)
            RETURNING user_id, coins, last_daily, last_work, inventory
            `,
            [String(userId), []]
        )
...
async function updateUser(userId, data) {
    try {
        ...
        const result = await databaseSystem.query(
            ...
            RETURNING user_id, coins, last_daily, last_work, inventory
            `,
            [String(userId), coins, lastDaily, lastWork, inventory]
        )
...
// ✅ daily فقط
async function updateDaily(userId, timestamp) {
    try {
        ...
        logger.error("ECONOMY_UPDATE_DAILY_FAILED", {
            error: error.message
        })
...
// ✅ work فقط (هذا اللي كان ناقصك)
async function updateWork(userId, timestamp) {
    try {
        ...
        logger.error("ECONOMY_UPDATE_WORK_FAILED", {
            error: error.message
        })
