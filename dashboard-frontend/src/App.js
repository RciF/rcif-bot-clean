  ...
  return (
    <div className="fade-in">
      ...
      <div className="card" style={{padding:18,marginBottom:16}}>
        <h3 style={{fontSize:12,color:"var(--muted)",marginBottom:12}}>الأقسام الرئيسية</h3>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:9}}>
          {quickBtns.map(s => {
            const meta = CAT_META[s.id]
            const locked = meta && !canUsePlan(guildPlan, meta.plan||"free")
            return (
              <button key={s.id} onClick={()=>!locked&&onSection(s.id)}
                className="btn btn-ghost"
                style={{justifyContent:"flex-start",gap:8,padding:"9px 12px",
                  border:`1px solid ${locked?"var(--border)":s.color+"44"}`,
                  color:locked?"var(--muted)":s.color,opacity:locked?.5:1}}>
      ...
      <div className="card" style={{padding:18}}>
        <h3 style={{fontSize:12,color:"var(--muted)",marginBottom:12}}>حالة الأنظمة</h3>
        {[
          {name:"الذكاء الاصطناعي", active:settings?.ai,      plan:"gold",   desc:`${PLANS.find(p=>p.id===guildPlan)?.ai_limit||0} رسالة/يوم`},
          ...
          <div key={s.name} style={{display:"flex",justifyContent:"space-between",alignItems:"center",
            ...
            <div style={{display:"flex",gap:11,alignItems:"center"}}>
              <div className="pulse-dot" style={{background:s.active?"var(--green)":"var(--red)",
                boxShadow:s.active?"0 0 7px var(--green)":"none"}}/>
              <div>
